...
#
# This file simulates the model scenario: constant value.
#
...
add <- "const" # type of increase in propagule pressure, required for output file; here constant

## Define temporal development of propagule pressure ##########################################################
m <- rev(exp(-seq(0,10,length.out=t_max))) # exponential increase
m <- rep(mean(m),t_max) # constant, uses only the mean of the increase in other model scenarios for comparison
