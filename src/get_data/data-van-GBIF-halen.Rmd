...
Caculate the cumulative number of invasive species per kingdom and total and display in a graph
```{r}
...
for(i in 1:tijdspanne){
  temp <- subset(species_list, firstyear == t)
  kingdoms <- unique(temp$kingdom)
  for(k in kingdoms){
  temp2 <- subset(temp, kingdom == k)
  temp3$n <- nrow(temp2)
  temp3$t <- t
  temp3$kingdom <- k
...
#Om de een of andere reden vult het bovenstaande script de tijdspanne niet volledig aan. Enkel jaren met nieuwe soorten worden aangevuld. => stuk script toevoegen om aan de vullen 

new_species_k$X1 <- NULL
new_species_k$cumul <- transform(new_species_k, kingdom = ave(n , kingdom, FUN = cumsum))

plot <- ggplot(new_species_k, aes(x = t, y=cumul, colour= kingdom))
plot <- plot + geom_line()
