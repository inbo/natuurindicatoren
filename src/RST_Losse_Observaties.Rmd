...
for(taal in talen){
  ...
  p <- p + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
  if(taal == "NL"){
    p <- p + scale_y_continuous("Maandmaximum in België o.b.v. losse waarnemingen",breaks = seq(0, 20, by = 2), limits = c(0,20), expand = c(0,0))
  }else{
    if(taal == "EN"){
      p <- p + scale_y_continuous("Monthly maximum in Belgium based on individual observations",breaks = seq(0, 20, by = 2), limits = c(0,20),expand = c(0,0))
    }else{
      print("WARNING: Translate y-axis!! defaults to english")
      p <- p + scale_y_continuous("Monthly maximum in Belgium based on individual observations",breaks = seq(0, 20, by = 2), limits = c(0,20),expand = c(0,0))
    }
