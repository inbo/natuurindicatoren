...
data_flanders_listed_class_nl <- 
  data_flanders_listed %>% 
  group_by(class_nl) %>% 
  summarise(n = n_distinct(key))
  
p <- ggplot(data = data_flanders_listed_class_nl, aes(x = reorder(class_nl, n), y = n))
p <- p + geom_bar(stat = "identity")
p <- p + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
p <- p + coord_flip()
p <- p + scale_y_continuous("", breaks = seq(0, 15, by = 2), minor_breaks = seq(0,15, by=1), expand = c(0,0))
