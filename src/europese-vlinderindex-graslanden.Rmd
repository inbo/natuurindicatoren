...
df = read.xls("Graslandvlinderindex_data2016_web.xlsx", sheet = 1, header = TRUE)
rownames(df) <- df$Soort
df$Soort <- NULL
colnames(df) <- substring(colnames(df), first = 2)
...
data_plot1 = df[1:7,]
data_plot1 <- tibble::rownames_to_column(data_plot1)
data_plot1 <- melt(data_plot1, id.vars = "rowname", 
                   value.name = "index", 
                   variable.name = "year")
data_plot1$year <- as.numeric(as.character(data_plot1$year))
ggplot(data_plot1, aes(x = year, y = index,
                       group = rowname)) +
            geom_point() +
            geom_line() +
            ...
            scale_x_continuous(pretty(data_plot1$year, n = 5))

```

## Evolutie van de vlinderindex

```{r, echo=FALSE, fig.width=10, fig.height=10}
data_plot2 = df[9:10,]
data_plot2 <- tibble::rownames_to_column(data_plot2)
data_plot2 <- melt(data_plot2, id.vars = "rowname", 
                   value.name = "index", 
                   variable.name = "year")
data_plot2$year <- as.numeric(as.character(data_plot2$year))
ggplot(data_plot2, aes(x = year, y = index*100,
                       group = rowname, colour = rowname)) +
            geom_point() +
            geom_line() +
            geom_hline(yintercept = 100, 
                       color = INBOdarkgreen, 
            ...
            theme(legend.title = element_blank(),
                  ...
                  text = element_text(12)) + 
            ylab("Graslandvlinder Index (100% defines situation in 1991)") +
            scale_x_continuous(pretty(data_plot2$year, n = 5))
```
...
data_plot3 <- df[13,]
MINA_plan_4_value <- df["doel MINA-plan 4", "2015"]

data_plot3 <- melt(data_plot3,
                   value.name = "index", 
                   variable.name = "year",
                   variable.factor = FALSE)
data_plot3$year <- as.numeric(as.character(data_plot3$year))
ggplot(data_plot3, aes(x = year, y = index)) +
            geom_point() +
            ...
            theme(legend.title = element_blank(),
                  ...
                  text = element_text(12)) + 
            ylab("Graslandvlinder Index (100% defines situation in 1991)") +
            scale_x_continuous(pretty(data_plot3$year, n = 5)) +
            xlab("")
