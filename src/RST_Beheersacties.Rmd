...
mapview::mapshot(map_split, file = paste0(getwd(),"./reports/figures/RST_Beheer_split.jpeg"), remove_controls = c("zoomControl", "layersControl", "homeButton"), remove_url = TRUE)

saveWidget(map_split, file="kaartje_beheersacties_split.html", selfcontained = TRUE)
file.copy("./kaartje_beheersacties_split.html", "./data/processed/kaartje_beheersacties_split.html")
file.remove("./kaartje_beheersacties_split.html")
```
...
saveWidget(map_samen, file="kaartje_beheersacties.html", selfcontained = TRUE)
...
```{r Tabel}
Jaren <- c(as.character(2009:max(Taxon_Rosse_Stekelstaart_int$Year)))
...
totaal <- Taxon_Rosse_Stekelstaart_int %>% 
  filter(!grepl(pattern = "Niet", Taxon_MeasurementQualifier)) %>% 
  group_by(Year) %>% 
  summarise(Aantal = sum(as.numeric(Taxon_Data), na.rm = TRUE)) %>% 
  right_join(y = temp)

for(j in Jaren){
  j <- as.character(j)
  Tabel[[j]] <- c(NA, 
                  afschot_am$Aantal[afschot_am$Year == j], 
                  afschot_av$Aantal[afschot_av$Year == j],
                  afschot_juv$Aantal[afschot_juv$Year == j],
                  NA, 
                  vangst_am$Aantal[vangst_am$Year == j],
                  vangst_av$Aantal[vangst_av$Year == j],
                  totaal$Aantal[totaal$Year == j])
}
...
#Tabel[,as.character(year(Sys.Date()))] <- NULL #Drop ongoing year

write_delim(Tabel, "./data/processed/Beheeracties_Tabel.csv", delim = ";", na = "")
```

```{r update statistieken}
RST_Statistieken <- read_delim("data/processed/RST_Statistieken.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE)

RST_Statistieken <- RST_Statistieken %>% 
  mutate(adult_afschot = sum(Tabel[2,ncol(Tabel)],Tabel[3,ncol(Tabel)], na.rm = TRUE),
         juveniel_afschot = sum(Tabel[4,ncol(Tabel)], na.rm = TRUE)) 
